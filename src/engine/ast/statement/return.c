...
Statement ReturnStatement_create() {
  ...
  statement->node->type = NT_ReturnStatement;
...
Statement readReturnStatement(SourceFile file, cstring source) {
  ...
  Token_dispose(token);
  Statement ret_state = ReturnStatement_create();
  ...
  if (token->type == TT_Newline || token->type == TT_MultiLineComment) {
    ...
    ret_state->return_.expression = body;
...
void ReturnStatement_dispose(Statement statement) {
  if (statement->return_.expression) {
    Expression_dispose(statement->return_.expression);
  }
